Shader "Hidden/Locked/.poiyomi/Poiyomi 8.2/Poiyomi Pro/ab3cb7c041def7146be2e92c0b1a2db4"
{
	Properties
	{
		[HideInInspector] shader_master_label ("<color=#E75898ff>Poiyomi 8.2.012</color>", Float) = 0
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				float MainColorFeatherStep = (0.753 /*_BaseColor_Step*/) - (0.113 /*_BaseShade_Feather*/);
				float firstColorFeatherStep = (0.098 /*_ShadeColor_Step*/) - (1.0 /*_1st2nd_Shades_Feather*/);
				#if defined(PROP_1ST_SHADEMAP) || !defined(OPTIMIZER_ENABLED)
				...
				float mainShadowMask = saturate(1 - ((poiLight.lightMap) - MainColorFeatherStep) / ((0.753 /*_BaseColor_Step*/) - MainColorFeatherStep) * (shadowMask));
				float firstSecondShadowMask = saturate(1 - ((poiLight.lightMap) - firstColorFeatherStep) / ((0.098 /*_ShadeColor_Step*/) - firstColorFeatherStep) * (shadowMask));
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#if defined(PROP_GLITTERMASK) || !defined(OPTIMIZER_ENABLED)
				float glitterMask = POI2D_SAMPLER_PAN(_GlitterMask, _MainTex, poiUV(poiMesh.uv[(0.0 /*_GlitterMaskUV*/)], float4(2,2,0,0)), float4(0,0,0,0))[(0.0 /*_GlitterMaskChannel*/)];
				#else
				float glitterMask = 1;
				#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if ((0.0 /*_LightingVertexLightingEnabled*/))
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						poiLight.vColor[index] = (0.0 /*_LightingAdditiveLimited*/) ? min((7.94 /*_LightingAdditiveLimit*/), unity_LightColor[index].rgb) : unity_LightColor[index].rgb;
				...
				poiLight.directColor = (0.0 /*_LightingAdditiveLimited*/) ? min((7.94 /*_LightingAdditiveLimit*/), _LightColor0.rgb) : _LightColor0.rgb;
				#if defined(POINT_COOKIE) || defined(DIRECTIONAL_COOKIE)
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.4865309,0.5124745,1,1).rgb, (0.0 /*_ColorThemeIndex*/));
				poiFragData.alpha = mainTexture.a * float4(0.4865309,0.5124745,1,1).a;
				#ifdef COLOR_GRADING_HDR
				#if defined(PROP_MAINCOLORADJUSTTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 hueShiftAlpha = POI2D_SAMPLER_PAN(_MainColorAdjustTexture, _MainTex, poiUV(poiMesh.uv[(0.0 /*_MainColorAdjustTextureUV*/)], float4(2,2,0,0)), float4(0,0,0,0));
				#else
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), -((3.58 /*_Saturation*/)) * hueShiftAlpha.b);
				poiFragData.baseColor = saturate(poiFragData.baseColor + (0.23 /*_MainBrightness*/) * hueShiftAlpha.g);
				#endif
